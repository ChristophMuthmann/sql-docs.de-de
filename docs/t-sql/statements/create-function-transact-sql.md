...
### <a name="d-creating-a-clr-function"></a>D. Erstellen einer CLR-Funktion  
 In diesem Beispiel wird die CLR-Funktion `len`_`s` erstellt. Bevor die Funktion erstellt wird, wird die Assembly `SurrogateStringFunction.dll` in der lokalen Datenbank registriert.  
