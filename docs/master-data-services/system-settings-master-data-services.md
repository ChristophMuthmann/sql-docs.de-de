---
title: "Systemeinstellungen (Master Data Services) | Microsoft Docs"
ms.custom: ""
ms.date: "03/01/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "Master Data Services, Systemeinstellungen"
  - "Systemeinstellungen [Master Data Services]"
...
|**Staging-Batchintervall**|**StagingBatchInterval**|Gibt im Funktionsbereich **Integrationsmanagement** des [!INCLUDE[ssMDSmdm](../includes/ssmdsmdm-md.md)] an, wie viele Sekunden nach der Auswahl von **Batches starten** der Batch verarbeitet wird. Der Standardwert ist **60** Sekunden (1 Minute).|  
...
|Einstellung des Konfigurations-Managers|Systemeinstellung|Description|  
|-----------------------------------|--------------------|-----------------|  
|**Die Standardanzahl der Elemente in der Hierarchie.**|**HierarchyChildNodeLimit**|Gibt im Funktionsbereich **Explorer** des [!INCLUDE[ssMDSmdm](../includes/ssmdsmdm-md.md)] an, wie viele Elemente maximal in den einzelnen Hierarchieknoten angezeigt werden, bevor **…More…** (Mehr) angezeigt wird angezeigt. Sie können auf **…More…** (Mehr) klicken,  um die nächste Gruppe von Elementen anzuzeigen. Der Standardwert lautet **50**.|  
|**Standardmäßig Namen in Hierarchie anzeigen**|**ShowNamesInHierarchy**|Legt im Funktionsbereich **Explorer** des [!INCLUDE[ssMDSmdm](../includes/ssmdsmdm-md.md)] die Standardeinstellung fest, die bei der Anzeige von Hierarchien ausgewählt wird.<br /><br /> Der Standardwert ist **Yes** oder **1**. Er gibt an, dass Name und Code der einzelnen Elemente angezeigt werden. Ändern Sie den Wert in **No** oder **2**, wenn Sie nur den Code anzeigen möchten.|  
|**Anzahl domänenbasierter Attribute in Liste**|**DBAListRowLimit**|Gibt an, wie viele Attribute in einer Liste im Funktionsbereich des [!INCLUDE[ssMDSmdm](../includes/ssmdsmdm-md.md)]-**Explorers** angezeigt werden, wenn Sie auf einen domänenbasierten Attributwert im Raster doppelklicken. Der Standardwert lautet **50**. Wenn mehr als 50 Elemente vorhanden sind, wird stattdessen ein durchsuchbares Dialogfeld angezeigt.|  
||**GridFilterDefaultFuzzySimilarityLevel**|Gibt im Funktionsbereich [!INCLUDE[ssMDSmdm](../includes/ssmdsmdm-md.md)]-**Explorer** an, welcher Ähnlichkeitsgrad bei Verwendung der Filterkriterien **Übereinstimmungen** verwendet wird. Der Standardwert ist **0,3**. Legen Sie als Wert eine Zahl in der Nähe von **1** fest, um ein Ergebnis zurückzugeben, das den Suchkriterien besser entspricht. Eine exakte Übereinstimmung erhalten Sie, wenn Sie den Wert auf **1** festlegen.|  
...
|**Anzahl von Elementen zur Anwendung von Geschäftsregeln.**|**BusinessRuleRealtimeMemberCount**|Gibt im Funktionsbereich [!INCLUDE[ssMDSmdm](../includes/ssmdsmdm-md.md)]-**Explorer** die maximale Anzahl von Elementen an, auf die im Raster Geschäftsregeln angewendet werden sollen. In [!INCLUDE[ssMDSXLS](../includes/ssmdsxls-md.md)] die maximale Anzahl von Elementen im aktiven Arbeitsblatt, auf die Geschäftsregeln angewendet werden sollen. Der Standardwert ist **10000**.|  
...
|Einstellung des Konfigurations-Managers|Systemeinstellung|Description|  
|-----------------------------------|--------------------|-----------------|  
|**Master Data Manager-URL für Benachrichtigungen**|**MDMRootURL**|Gibt die URL für die [!INCLUDE[ssMDSmdm](../includes/ssmdsmdm-md.md)]-Webanwendung an, die im Link in E-Mail-Benachrichtigungen verwendet wird (beispielsweise http://constoso/mds).|  
...
|**Regulärer Ausdruck für E-Mail-Adresse**|**EmailRegExPattern**|Gibt im Funktionsbereich [!INCLUDE[ssMDSmdm](../includes/ssmdsmdm-md.md)]-**Benutzer- und Gruppenberechtigungen** den regulären Ausdruck an, mit dem die auf der Registerkarte **Allgemein** eines Benutzers gespeicherte E-Mail-Adresse geprüft wird. Weitere Informationen zu regulären Ausdrücken finden Sie unter [Sprachelemente für reguläre Ausdrücke](http://go.microsoft.com/fwlink/?LinkId=164401) in der MSDN Library.|  
