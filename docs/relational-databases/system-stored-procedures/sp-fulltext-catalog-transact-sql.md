...
## <a name="arguments"></a>Argumente  
 ...
 Der Name des Volltextkatalogs. Katalognamen müssen für jede Datenbank eindeutig sein. *fulltext_catalog_name* is **sysname**.  
...
|**Drop**|Löscht *Fulltext_catalog_name* aus dem Dateisystem entfernt wird, und löschen die zugehörige Zeile in **Sysfulltextcatalogs**. Diese Aktion schlägt fehl, wenn der Katalog Indizes für eine oder mehrere Tabellen enthält. **sp_fulltext_table** '*table_name*', 'drop' should be executed to drop the tables from the catalog.<br /><br /> Wenn der Katalog nicht vorhanden ist, wird ein Fehler gemeldet.|  
