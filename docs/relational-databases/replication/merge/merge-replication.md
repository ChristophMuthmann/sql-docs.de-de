---
title: "Mergereplikation | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "Mergereplikation [SQL Server-Replikation], Informationen zur Mergereplikation"
  - "Mergereplikation [SQL Server-Replikation]"
...
-   Die Anwendung muss nicht auf die einzelnen Zwischenstufen von Datenänderungen, sondern nur auf das endgültige Ergebnis der Datenänderung zugreifen können. Wenn sich eine Zeile auf dem Abonnenten z. B. fünfmal ändert, bevor sie mit einem Verleger synchronisiert wird, ändert sich die Zeile auf dem Verleger lediglich einmal und übernimmt so nur die endgültige Änderung (in diesem Fall also den fünften Wert).  
 ...
 Damit Änderungen nachverfolgt werden können, müssen Mergereplikationen (und Transaktionsreplikationen) mit Abonnements mit verzögertem Update jede Zeile in jeder veröffentlichten Tabelle eindeutig identifizieren können. Mergereplikation-Spalte hinzugefügt **Rowguid** zu jeder Tabelle, wenn die Tabelle bereits eine Spalte vom Datentyp wurde **Uniqueidentifier** mit der **ROWGUIDCOL** -Eigenschaft (in diesem Fall wird diese Spalte verwendet). Beim Löschen der Tabelle aus der Veröffentlichung wird auch die **rowguid** -Spalte entfernt. Wurde eine vorhandene Spalte zur Nachverfolgung verwendet, wird die Spalte nicht entfernt. Ein Filter muss die von der Replikation verwendete **rowguidcol** nicht einschließen, um Zeilen zu identifizieren. Die **newid()** Funktion dient als Standardwert für die **Rowguid** Spalte jedoch Kunden eine Guid für jede Zeile können bei Bedarf bereitstellen. Geben Sie jedoch nicht Wert 00000000-0000-0000-0000-000000000000.  
  
 Im folgenden Diagramm werden die in der Mergereplikation verwendeten Komponenten dargestellt.  
  
 ![Komponenten und Datenfluss für Mergereplikation](../../../relational-databases/replication/merge/media/merge.gif "Komponenten und Datenfluss für Mergereplikation")  
