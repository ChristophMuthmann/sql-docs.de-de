---
title: "Konfigurieren von IIS f&#252;r die Websynchronisierung | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
helpviewer_keywords: 
  ...
  - "websync.log"
  - "Websynchronisierung, IIS-Server"
...
---
# Konfigurieren von IIS f&#252;r die Websynchronisierung
...
-   Replikation [!INCLUDE[tsql](../../includes/tsql-md.md)] programming: [Gewusst wie: Konfigurieren eines Abonnements für die Websynchronisierung verwenden (Replication Transact-SQL Programming)](http://msdn.microsoft.com/library/ms345206.aspx)  
  
-   RMO: [Gewusst wie: Konfigurieren eines Abonnements für die Verwendung der Websynchronisierung (RMO-Programmierung)](http://msdn.microsoft.com/library/ms345207.aspx)  
...
#### So geben Sie ein Zertifikat für die Bereitstellung an  
  
1.  Melden Sie sich an dem Computer mit IIS als Administrator an.  
  
2.  Starten Sie **Internetinformation Services (IIS)-Manager**:  
...
3.  Führen Sie den IIS-Zertifikat-Assistenten aus:  
    ...
    1.  Im **Internet Information Services (IIS) Manager**, erweitern Sie die **lokalen Computer** Knoten, und erweitern Sie dann die **Websites** Ordner.  
  
    2.  Mit der rechten Maustaste **Default Web Site**, und klicken Sie dann auf **Eigenschaften**.  
...
#### So geben Sie ein Zertifikat für das Testen in IIS 6.0 an  
  
1.  Melden Sie sich an dem Computer mit IIS als Administrator an.  
  
2.  Laden Sie SelfSSL herunter, und installieren Sie es. Standardmäßig befindet sich die Anwendung zu \<*Laufwerk*>: \programme\iis Resources\SelfSSL. Anwendungs- und dokumentationsverknüpfungen werden in \<*Laufwerk*>: \Documents und Einstellungen\All users\startmenü\programme\iis resources\selfssl kopiert.  
...
## Installieren der Konnektivitätskomponenten und von SQL Server Management Studio  
  
#### So installieren Sie die SQL Server-Konnektivitätskomponenten und SQL Server Management Studio  
  
1.  Melden Sie sich an dem Computer mit IIS als Administrator an.  
  
2.  Starten Sie den Installations-Assistenten für [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] vom [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] -Installationsdatenträger. Weitere Informationen zum Verwenden dieses Assistenten finden Sie unter [Installieren Sie SQL Server 2016 von der Installations-Assistent & #40; Setup & #41;](../../database-engine/install-windows/install-sql-server-2016-from-the-installation-wizard-setup.md).  
  
3.  Wählen Sie auf der Seite **Funktionsauswahl** **Konnektivität der Clienttools**aus.  
  
4.  Wenn Sie den Assistenten zum Konfigurieren der Synchronisierung verwenden möchten, wählen Sie **Verwaltungstools - einfach**.  
...
## Konfigurieren des Computers mit IIS mithilfe des Assistenten zum Konfigurieren der Websynchronisierung  
...
-   Erstellt ein virtuelles Verzeichnis und den zugeordneten Alias. Der Alias wird für den Zugriff auf die Websynchronisierungskomponenten verwendet. Wenn die IIS-Adresse https:// ist z. B.*server.domain.com* und Sie einen Alias 'websync1' angeben, die Adresse, die die Komponente replisapi.dll zugegriffen wird, https://*server.domain.com*/websync1/replisapi.dll.  
...
#### So konfigurieren Sie den Computer mit IIS mithilfe des Assistenten zum Konfigurieren der Websynchronisierung  
...
3.  Erweitern Sie die **lokale Publikationen** Ordner mit der rechten Maustaste in der Publikation, und klicken Sie dann auf **Configure Web Synchronization**.  
...
6.  Gehen Sie auf der Seite **Informationen zum virtuellen Verzeichnis** wie folgt vor:  
    ...
    2.  Geben Sie im Feld **Pfad** einen Pfad für das virtuelle Verzeichnis ein. Beispielsweise, wenn der eingegebene **websync1** in der **Alias** Geben **C:\Inetpub\wwwroot\websync1** in der **Pfad** Feld. Klicken Sie auf **Weiter**.  
...
8.  Gehen Sie auf der Seite **Verzeichniszugriff** wie folgt vor:  
    ...
    1.  Klicken Sie auf **Hinzufügen**, und fügen Sie dann im Dialogfeld **Benutzer oder Gruppen auswählen** die Konten hinzu, über die die Abonnenten die Verbindungen zu IIS herstellen werden. Diese sind die Konten, die Sie auf der **Webserverinformationen** Seite des Assistenten für neue Abonnements oder als Wert für die [Sp_addmergepullsubscription_agent](../../relational-databases/system-stored-procedures/sp-addmergepullsubscription-agent-transact-sql.md)*@internet_login* Parameter.  
  
9. Geben Sie auf der Seite **Zugriff auf Momentaufnahmefreigabe** die Momentaufnahmefreigabe ein. Die entsprechenden Berechtigungen für diese Freigabe werden so erteilt, dass die Abonnenten auf die Momentaufnahmedateien zugreifen können. Weitere Informationen zu Berechtigungen für die Freigabe, finden Sie unter [Sichern des Momentaufnahmeordners](../../relational-databases/replication/security/secure-the-snapshot-folder.md).  
...
## Manuelles Konfigurieren des Computers, auf dem IIS ausgeführt wird  
...
1.  Erstellen Sie ein Dateiverzeichnis auf dem Computer mit IIS, in dem die Datei replisapi.dll enthalten sein soll. Wo sollen, aber es wird empfohlen, dass Sie das Verzeichnis erstellen, erstellen Sie das Verzeichnis kann die \<*Laufwerk*>: \Inetpub-Verzeichnis. Erstellen Sie beispielsweise das Verzeichnis \<*Laufwerk*>: \Inetpub\SQLReplication\\.  
...
1.  Erstellen Sie ein lokales Konto auf dem Computer mit IIS:  
    ...
    1.  Mit der rechten Maustaste **Arbeitsplatz**, und klicken Sie dann auf **verwalten**.  
...
2.  Fügen Sie das Konto der IIS_WPG-Gruppe hinzu:  
    ...
    2.  Mit der rechten Maustaste **IIS_WPG**, und klicken Sie dann auf **Gruppe hinzufügen**.  
  
    3.  In der **Eigenschaften von IIS_WPG** Dialogfeld klicken Sie auf **Hinzufügen**.  
    ...
    6.  In der **Benutzer** Dialogfeld und der **Eigenschaften von IIS_WPG** im Dialogfeld klicken Sie auf **OK**.  
  
3.  Erteilen Sie dem Konto die mindestens erforderlichen Berechtigungen für den Ordner, der die Datei replisapi.dll enthält:  
  
    1.  Suchen Sie den Ordner, die Sie für die Datei replisapi.dll erstellt, mit der rechten Maustaste in des Ordners, und klicken Sie dann auf **Freigabe und Sicherheit**.  
...
4.  Erstellen eines Anwendungspools in **Internet Information Services (IIS) Manager**:  
    ...
    3.  Im **Internet Information Services (IIS) Manager**, erweitern Sie die **lokalen Computer** Knoten.  
  
    4.  Mit der rechten Maustaste **Anwendungspools**, zeigen Sie auf **Neu** und klicken Sie dann auf **Anwendungspool**.  
...
5.  Ordnen Sie das Konto dem Anwendungspool zu:  
    ...
    1.  Im **Internet Information Services (IIS) Manager**, erweitern Sie die **lokalen Computer** Knoten, und erweitern Sie dann **Anwendungspools**.  
  
    2.  Mit der rechten Maustaste des Anwendungspools, den Sie erstellt haben, und klicken Sie dann auf **Eigenschaften**.  
  
    3.  In der **\< ApplicationPoolName> Eigenschaften** Dialogfelds die **Identität** auf **konfigurierbar**.  
...
6.  Ordnen Sie den Anwendungspool dem virtuellen Verzeichnis zu, das für die Websynchronisierung verwendet wird:  
    ...
    1.  Im **Internet Information Services (IIS) Manager**, erweitern Sie die **lokalen Computer** Knoten, und erweitern Sie dann **Websites**.  
  
    2.  Erweitern Sie die Website, die Sie für die websynchronisierung verwenden, mit der rechten Maustaste in des virtuellen Verzeichnis, das Sie für die Synchronisierung von erstellt haben, und klicken Sie dann auf **Eigenschaften**.  
  
    3.  Auf der **virtuelles Verzeichnis** auf der Registerkarte der **\< VirtualDirectoryName> Eigenschaften** im Dialogfeld aus der **Anwendungspool** Dropdown-wählen Sie den Anwendungspool, der in Schritt 5 erstellt wurde.  
