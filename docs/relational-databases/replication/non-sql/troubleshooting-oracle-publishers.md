---
title: "Problembehandlung bei Oracle-Verlegern | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "Veröffentlichungen mit Oracle [SQL Server-Replikation], Problembehandlung"
  - "Problembehandlung [SQL Server-Replikation], Veröffentlichungen mit Oracle"
...
## Fehler bezüglich Oracle-Client- und Netzwerksoftware  
 Das Konto, unter dem [!INCLUDE[msCoName](../../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] auf dem Verteiler ausgeführt wird, müssen werden gewährt Lese- und Ausführungsberechtigungen für das Verzeichnis (und alle Unterverzeichnisse), in dem die Oracle-Clientnetzwerksoftware installiert ist. Wenn diese Berechtigungen nicht erteilt worden sind oder die Oracle-Clientkomponenten nicht ordnungsgemäß installiert sind, wird eine Fehlermeldung ähnlich der folgenden angezeigt:  
...
-   Der Anmeldename, den die Replikation zur Herstellung einer Verbindung mit dem Oracle-Verleger verwendet, verfügt über ausreichende Berechtigungen. Weitere Informationen finden Sie unter [einen Oracle-Verleger konfigurieren](../../../relational-databases/replication/non-sql/configure-an-oracle-publisher.md).  
...
-   Das richtige Oracle Home-Verzeichnis und der entsprechende Pfad werden verwendet. Auch wenn Sie nur einen einzigen Satz von Oracle-Binärdateien auf dem [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] -Verteiler installiert haben, muss sichergestellt sein, dass die Umgebungsvariablen für das Oracle Home-Verzeichnis ordnungsgemäß festgelegt wurden. Wenn Sie Werte für Umgebungsvariablen ändern, müssen Sie [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] beenden und neu starten, damit die Änderungen wirksam werden.  
 ...
 Weitere Informationen zum Konfigurieren und Testen der Konnektivität finden Sie unter "installieren und Konfigurieren von Oracle-Clientnetzwerksoftware auf die [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Verteiler" im [Konfigurieren eines Oracle-Verlegers](../../../relational-databases/replication/non-sql/configure-an-oracle-publisher.md).  
...
-   "Die Oracle-Serverinstanz ' \<*OraclePublisherName*>' wurde zuvor zur Verwendung konfiguriert ' \<*SQLServerDistributorName*>' als Verteiler. Beginnen mit "\<*NewSQLServerDistributorName*>' als Verteiler, müssen Sie die aktuelle Replikationskonfiguration auf der Oracle-Serverinstanz, in dem alle Veröffentlichungen auf dieser Serverinstanz gelöscht entfernen."  
  
-   "Oracle-Server ' \<*OracleServerName*>' ist bereits als Verleger definiert ' \<*OraclePublisherName*>' auf dem Verteiler ' \<*SQLServerDistributorName*>.*\< DistributionDatabaseName>*". Löschen Sie den Verleger, oder löschen Sie das öffentliche Synonym '*\< SynonymName>*', neu zu erstellen. "  
...
## SQL Server-Fehler 21663 (Fehlen des Primärschlüssels) wurde ausgelöst  
 ...
 "Kein gültiger Primärschlüssel gefunden, für die Quelltabelle [\<*TableOwner*>]. [ \<*TableName*>] "  
...
## SQL Server-Fehler 21642 (Verbindungsserver bereits vorhanden) wurde ausgelöst  
 ...
 "Für heterogene Verleger ist ein Verbindungsserver erforderlich. Ein Verbindungsserver namens "*\< LinkedServerName>*' ist bereits vorhanden. Entfernen Sie den Verbindungsserver, oder wählen Sie einen anderen Verlegernamen aus."  
  
 Dieser Fehler kann auftreten, wenn Sie versuchen, den Verbindungsserver direkt zu erstellen, oder die Beziehung zwischen dem Oracle-Verleger und dem [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] -Verteiler zuvor gelöscht haben und jetzt versuchen, diese Beziehung neu zu konfigurieren. Wenn Sie diese Fehlermeldung beim Versuch, den Verleger neu konfigurieren, legen Sie den Verbindungsserver mit [Sp_dropserver & #40; Transact-SQL & #41;](../../../relational-databases/system-stored-procedures/sp-dropserver-transact-sql.md).  
  
 Wenn Sie mit dem Oracle-Verleger über einen Verbindungsserver eine Verbindung herstellen möchten, erstellen Sie einen anderen TNS-Dienstnamen und verwenden Sie diesen Namen, wenn der Aufruf von [Sp_addlinkedserver & #40; Transact-SQL & #41;](../../../relational-databases/system-stored-procedures/sp-addlinkedserver-transact-sql.md). Informationen zum Erstellen von TNS-Dienstnamen finden Sie in der Oracle-Dokumentation.  
  
## SQL Server-Fehler 21617 wurde ausgelöst  
 Für das Veröffentlichen mit Oracle wird die Oracle-Anwendung SQL*PLUS verwendet, um das Paket mit Unterstützungscode für den Verleger in die Oracle-Datenbank herunterzuladen. Bevor der Oracle-Verleger konfiguriert [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] stellt sicher, dass SQL\*PLUS ist über den Systempfad auf dem Verteiler zugegriffen werden kann. Wenn SQL\*PLUS kann nicht geladen werden kann, wird die folgende Fehlermeldung angezeigt:  
 ...
 Suchen Sie nach SQL\*PLUS auf dem Verteiler. Bei einer Oracle 10g-Clientinstallation lautet der Name dieser ausführbaren Datei sqlplus.exe. Sie ist normalerweise in %ORACLE_HOME%/bin installiert. Überprüfen, ob der Pfad von SQL\*PLUS erscheint im Systempfad, überprüfen Sie den Wert der Systemvariablen **Pfad**:  
  
1.  Mit der rechten Maustaste **Arbeitsplatz**, und klicken Sie dann auf **Eigenschaften**.  
...
5.  Klicken Sie in jedem geöffneten Dialogfeld auf **OK** , um den Vorgang zu beenden und die Änderungen zu speichern.  
 ...
 Wenn Sie sqlplus.exe auf dem Verteiler nicht finden können, installieren Sie die aktuelle Version der Oracle-Clientsoftware auf dem Verteiler. Weitere Informationen finden Sie unter [einen Oracle-Verleger konfigurieren](../../../relational-databases/replication/non-sql/configure-an-oracle-publisher.md).  
...
-   "Der CLSID-Registrierungsschlüssel, der anzeigt, dass der OLE DB-Anbieter von Oracle, OraOLEDB.Oracle, registriert wurde, ist auf dem Verteiler nicht vorhanden. Stellen Sie sicher, dass der OLE DB-Anbieter von Oracle auf dem Verteiler installiert und registriert ist."  
 ...
 Wenn Sie die Version 10g der Oracle-Clientsoftware verwenden, lautet der Dateiname des Anbieters OraOLEDB10.dll. Für Version 9i lautet er OraOLEDB.dll. Der Anbieter wird in %ORACLE_HOME%\BIN installiert (beispielsweise C:\oracle\product\10.1.0\Client_1\bin). Wenn Sie feststellen, dass der OLE DB-Anbieter von Oracle nicht auf dem Verteiler installiert ist, installieren Sie ihn über die von Oracle bereitgestellte Installations-CD mit der Oracle-Clientsoftware. Weitere Informationen finden Sie unter [einen Oracle-Verleger konfigurieren](../../../relational-databases/replication/non-sql/configure-an-oracle-publisher.md).  
...
3.  Navigieren Sie zu HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Microsoft SQLServer\\*\< InstanceName>*\Providers.  
...
4.  Wenn Sie feststellen, dass für **AllowInProcess** der Wert **0**festgelegt ist, ändern Sie den Registrierungseintrag auf **1**:  
    ...
    1.  Maustaste auf den Eintrag, und klicken Sie dann auf **Ändern**.  
...
## Berechtigungsprobleme beim Benutzerschema für die Replikation  
 Das Benutzerschema der Replikation müssen die Berechtigungen unter "Manuelles Erstellen des Benutzers Schema" in beschriebenen [Konfigurieren eines Oracle-Verlegers](../../../relational-databases/replication/non-sql/configure-an-oracle-publisher.md).  
...
## Oracle-Fehler ORA-01000  
 ...
 Zur Vermeidung dieses Problems sicher, dass die **Max_open_cursors** Einstellung in der Oracle-Datenbanken auf einen ausreichend hohen Wert (mindestens 1000) festgelegt wurde. Weitere Informationen zu dieser Einstellung finden Sie in der Oracle-Dokumentation.  
...
2.  Mit der rechten Maustaste **Replikation**, und klicken Sie dann auf **Verteilereigenschaften**.  
...
-   Führen Sie **Sp_dropdistpublisher**. Weitere Informationen finden Sie unter [Sp_dropdistpublisher & #40; Transact-SQL & #41;](../../../relational-databases/system-stored-procedures/sp-dropdistpublisher-transact-sql.md).  
