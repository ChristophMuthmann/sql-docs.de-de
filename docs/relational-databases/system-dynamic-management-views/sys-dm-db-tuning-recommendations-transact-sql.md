...
| **Typ** | **nvarchar(4000)** | Der Name der automatischen Optimierung Option, die die Empfehlung, z. B. erzeugt,`FORCE_LAST_GOOD_PLAN` |
| **Grund** | **nvarchar(4000)** | Der Grund, warum diese Empfehlung bereitgestellt wurde. |
| **gültige\_seit** | **datetime2** | Beim ersten Start wurde diese Empfehlung generiert. |
| **letzte\_aktualisieren** | **datetime2** | Zeitpunkt der letzten wurde diese Empfehlung generiert. |
| **Status** | **nvarchar(4000)** | JSON-Dokument, das den Zustand der Empfehlung beschreibt. Folgende Felder sind verfügbar:<br />-   `currentValue`-aktuellen Status der Empfehlung.<br />-   `reason`– Konstante, die beschreibt, warum die Empfehlung im aktuellen Zustand ist.|
...
| **Führen Sie\_Aktion\_initiiert\_durch** | **nvarchar(4000)** | `User`= Benutzer erzwungen manuell Plan in der Empfehlung. <br /> `System`= System wird automatisch Empfehlung angewendet. |
...
| **Bewertung** | **int** | Für diese Empfehlung für den 0-100 Wert/Auswirkungen geschätzte Skalierung (je größer die besser) |
