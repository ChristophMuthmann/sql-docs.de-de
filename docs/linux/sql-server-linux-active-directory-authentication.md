...
Führen Sie den folgenden Befehl zum Überprüfen, ob die [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] Hostcomputer ist so konfiguriert, dass für AD-Domänencontroller als ein DNS-Namenserver verwenden:
...
## <a name="step-2-create-ad-user-for-includessnoversionincludesssnoversion-mdmd-and-set-spn"></a>Schritt 2: Erstellen von AD-Benutzers für [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] und Festlegen des SPN
...
## <a name="step-3-configure-includessnoversionincludesssnoversion-mdmd-service-keytab"></a>Schritt 3: Konfigurieren von [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] Dienst Keytab-Datei
...
Jede Person mit Zugriff auf diese `keytab` Datei kann die Identität [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] in der Domäne, weshalb Sie sicherstellen müssen Sie den Zugriff auf die Datei, nur die `mssql` Konto über Lesezugriff verfügt:
...
Als Nächstes konfigurieren [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] dieses `keytab` Datei für die Kerberos-Authentifizierung:
...
Herstellen einer Verbindung mit [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] und erstellen Sie eine neue, auf AD basierende Anmeldung:

```sql
CREATE LOGIN [CONTOSO\user] FROM WINDOWS;
```

Stellen Sie sicher, dass die Anmeldung jetzt im aufgeführt ist die [Sys. server_principals](../relational-databases/system-catalog-views/sys-server-principals-transact-sql.md) Systemkatalogsicht:

```sql
SELECT name FROM sys.server_principals;
```

## <a name="step-5-connect-to-includessnoversionincludesssnoversion-mdmd-using-ad-authentication"></a>Schritt 5: Verbinden mit [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] mithilfe der AD-Authentifizierung
...
* ADO.NET: [Verbindungszeichenfolgensyntax](https://docs.microsoft.com/dotnet/framework/data/adonet/connection-string-syntax)
