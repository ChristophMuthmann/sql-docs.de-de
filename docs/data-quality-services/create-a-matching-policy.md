---
title: "Erstellen einer Abgleichsrichtlinie | Microsoft Docs"
ms.custom: ""
ms.date: "03/01/2017"
...
3.  Klicken Sie auf **Wissensdatenbank öffnen** , um die Abgleichsrichtlinie in einer vorhandenen Wissensdatenbank zu erstellen oder zu ändern. Wählen Sie die Wissensdatenbank aus, wählen Sie **Abgleichsrichtlinie**aus, und klicken Sie dann auf **Weiter**. Sie können auch unter **Zuletzt verwendete Wissensdatenbank**auf eine Wissensdatenbank klicken. Wenn Sie eine Wissensdatenbank, die geschlossen wurde öffnen, während eine Abgleichsrichtlinie gearbeitet wurde, fahren Sie fort mit der Phase, die die abgleichsrichtlinienaktivität geschlossen wurde (wie durch die **Zustand** Spalte für die Wissensdatenbank in der Knowledge Base-Tabelle oder den Namen der Knowledge Base unter **zuletzt verwendete Wissensdatenbank**). Wenn Sie eine Wissensdatenbank öffnen, die eine Abgleichsrichtlinie enthält und abgeschlossen wurde, wechseln Sie zur Seite **Abgleichsrichtlinie** . Wenn Sie eine Wissensdatenbank öffnen, die keine Abgleichsrichtlinie enthält und abgeschlossen wurde, wechseln Sie zur Seite **Zuordnung** .  
...
1.  Auf der **Zuordnung** Seite, um eine Richtlinie für eine Datenbank erstellen zu lassen **Datenquelle** als **SQL Server**, wählen Sie die Datenbank, die zum Erstellen der Richtlinie für in sollen **Datenbank**, und wählen Sie dann die Tabelle oder in anzeigen **Tabelle/Sicht**. Die Quelldatenbank muss sich in der gleichen SQL Server-Instanz wie [!INCLUDE[ssDQSServer](../includes/ssdqsserver-md.md)]befinden. Andernfalls wird sie nicht in der Dropdownliste angezeigt.  
...
5.  Klicken Sie auf der **plus (+)** Steuerelement eine Zeile der Zuordnungstabelle hinzu oder **minus (-)** -Steuerelement eine Zeile zu entfernen.  
...
7.  Klicken Sie auf **Verbunddomänen anzeigen/auswählen** eine Liste der in der Wissensdatenbank verfügbaren verbunddomänen anzuzeigen, und wählen Sie Optionen für die Zuordnung.  
...
3.  Vergrößern Sie den Wert für **Minimale Treffergenauigkeit** , wenn Sie die Abgleichsanforderungen strenger machen möchten. Weitere Informationen über die minimale treffergenauigkeit finden Sie unter [wie Festlegen von Parametern übereinstimmenden Regel](#MatchingRules).  
...
6.  Wählen Sie für **Ähnlichkeit**die Option **Ähnlich** aus, wenn zwei Werte im gleichen Feld von zwei verschiedenen Datensätzen auch dann als übereinstimmend angesehen werden können, wenn sie nicht identisch sind. Wählen Sie **Genau** aus, wenn zwei Werte im gleichen Feld von zwei verschiedenen Datensätzen identisch sein müssen, um als übereinstimmend angesehen zu werden. (Weitere Informationen finden Sie unter [Gewusst wie: Legen Sie Abgleichsregelparameter](#MatchingRules).)  
...
13. Sie können in der Tabelle **Abgleichsergebnisse** wie folgt mit den Daten arbeiten:  
    ...
    -   In der **Prozent**, wählen Sie einen Prozentsatz aus der Dropdown-Liste, in Schritten von "5". Alle Zeilen mit einer Treffergenauigkeit, die größer oder gleich diesem Prozentwert ist, werden in der Abgleichsergebnistabelle angezeigt.  
  
    -   Wenn Sie einen Datensatz in der abgleichsergebnistabelle doppelklicken, zeigt DQS eine **Details zur Treffergenauigkeit** Popupfenster, das den pivotdatensatz und Quelldatensatz (und die Werte in allen ihren Feldern), zeigt das Ergebnis zwischen ihnen und ein Drilldown des datensatzabgleichs. Der Drilldown zeigt die Werte in jedem Feld des Pivotdatensatzes und Quelldatensatzes an, damit Sie sie vergleichen können, und zeigt die Treffergenauigkeit an, die jedes Feld zur Gesamttreffergenauigkeit für die zwei Datensätze beiträgt.  
...
1.  Auf der **Abgleichsergebnisse** Seite **überlappende Cluster** aus der Dropdown-Liste an, die pivotdatensätze und die folgenden Datensätze für alle Cluster Wenn Abgleich ausgeführt wird, auch wenn Gruppen von Clustern gemeinsame Datensätze aufweisen. Wählen Sie **Nicht überlappende Cluster** aus, um Cluster anzuzeigen, die als einzelne Cluster gemeinsame Datensätze aufweisen, wenn der Abgleich ausgeführt wird.  
  
2.  Klicken Sie auf **erneut aus Datenquelle laden** zum Kopieren von Daten aus der Datenquelle in die Stagingtabelle und neu zu indizieren, wenn Sie die Abgleichsrichtlinie ausführen. Klicken Sie auf **für vorherige Daten ausführen** eine Abgleichsrichtlinie auszuführen, ohne die Daten in die Stagingtabelle zu kopieren und neu zu indizieren. **Für vorherige Daten ausführen** ist für die erste Ausführung der Abgleichsrichtlinie deaktiviert. Dies gilt auch, wenn Sie die Zuordnung auf der Seite **Struktur** ändern und in der folgenden Meldung auf **Ja** klicken. In beiden Fällen müssen Sie die Daten erneut indizieren. Die Neuindizierung ist nicht erforderlich, wenn die Abgleichsrichtlinie nicht geändert wurde. Durch die Ausführung für vorherige Daten kann die Leistung verbessert werden.  
  
3.  Klicken Sie auf **Start** , um den Abgleichsprozess für alle von Ihnen definierten Regeln auszuführen. Die **übereinstimmenden Ergebnisse** Tabelle zeigt die Datensatz-ID, Clusternummer und Datenspalten (einschließlich der nicht in der Abgleichsregel befinden) für jeden Datensatz in einem Cluster. Der führende Datensatz im Cluster wird zufällig ausgewählt. (Sie bestimmen den überdauernden Datensatz durch auswählen die Survivorship-Regel auf der **Exportieren Sie** Seite, wenn Sie das abgleichsprojekt ausführen.) Jede zusätzliche Zeile in einem Cluster wird als Duplikat angesehen; seine Treffergenauigkeit (im Vergleich zum Pivotdatensatz) wird in der Ergebnistabelle bereitgestellt.  
...
8.  Klicken Sie auf **Schließen** , um die Arbeit zu speichern, und kehren Sie zur DQS-Startseite zurück. Der Status der Wissensdatenbank zeigt die Zeichenfolge „Abgleichsrichtlinie - “ und den aktuellen Status an. Wenn Sie geklickt haben **Schließen** während Sie auf die **Abgleichsergebnis** -Bildschirm, der Status wird angezeigt: "Abgleichsrichtlinie - Ergebnisse". Wenn Sie auf Schließen, während Sie auf die **Abgleichsrichtlinie** Bildschirm der Zustand wird angezeigt: "Abgleich - Abgleichsrichtlinie". Nachdem Sie auf **Schließen**geklickt haben, um die **Wissensermittlungsaktivität** auszuführen, müssen Sie zur Aktivität **Abgleichsrichtlinie** zurückkehren, auf **Fertig stellen**klicken und dann auf **Ja** klicken, um die Wissensdatenbank zu veröffentlichen, oder auf **Nein** , um die Arbeit an der Wissensdatenbank zu speichern und zu beenden.  
