...
ms.topic: "article"
helpviewer_keywords: 
  - "Anweisungen [SQL Server], Eingabeaufforderung"
  - "QUIT-Befehl"
  - "Transact-SQL-Anweisungen, Eingabeaufforderung"
  - "EXIT-Befehl"
  - "sqlcmd-Befehle"
  - "ED-Befehl"
  - "sqlcmd (Hilfsprogramm)"
  - "Eingabeaufforderungs-Hilfsprogramme [SQL Server], sqlcmd"
  - "!! Befehl"
  - "gespeicherte Prozeduren [SQL Server], Eingabeaufforderung"
  - "gespeicherte Systemprozeduren [SQL Server], Eingabeaufforderung"
  - "sqlcmd-Hilfsprogramm, Informationen zum Hilfsprogramm sqlcmd"
  - "Skripts [SQL Server], Eingabeaufforderung"
  - "RESET-Befehl"
  - "GO-Befehl"
...
[!INCLUDE[tsql-appliesto-ss2008-asdb-asdw-pdw-_md](../includes/tsql-appliesto-ss2008-asdb-asdw-pdw-md.md)]
  ...
  Mit dem Hilfsprogramm **sqlcmd** können Sie an der Eingabeaufforderung, im **Abfrage-Editor** im SQLCMD-Modus, in einer Windows-Skriptdatei oder in einem Auftragsschritt des Betriebssystems (Cmd.exe) eines [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)]-Agent-Auftrags Transact-SQL-Anweisungen, Systemprozeduren und Skriptdateien eingeben. Dieses Hilfsprogramm verwendet zum Ausführen von [!INCLUDE[tsql](../includes/tsql-md.md)]-Batches ODBC. 
  
> [!NOTE]
> Die neueste Version des Hilfsprogramms „sqlcmd“ ist als Webversion aus dem [Download Center](http://go.microsoft.com/fwlink/?LinkID=825643) verfügbar. Mindestens Version 13.1 ist erforderlich, damit Always Encrypted (`-g`) und Azure Active Directory-Authentifizierung (`-G`) unterstützt werden. (Möglicherweise haben Sie mehrere Versionen von „sqlcmd.exe“ auf Ihrem Computer installiert. Achten Sie darauf, dass Sie die richtige Version verwenden. Um die Version zu bestimmen, führen Sie `sqlcmd -?` aus.)
...
> [!IMPORTANT] [!INCLUDE[ssManStudioFull](../includes/ssmanstudiofull-md.md)] (SSMS) verwendet den [!INCLUDE[msCoName](../includes/msconame-md.md)][!INCLUDE[dnprdnshort](../includes/dnprdnshort-md.md)]SqlClient für die Ausführung im regulären und im SQLCMD-Modus des **Abfrage-Editors**. Beim Ausführen von **sqlcmd** über die Befehlszeile verwendet **sqlcmd** den ODBC-Treiber. Da unterschiedliche Standardoptionen gelten können, führt die Ausführung derselben Abfrage im SQLCMD-Modus von [!INCLUDE[ssManStudioFull](../includes/ssmanstudiofull-md.md)] und im Hilfsprogramm **sqlcmd** möglicherweise zu unterschiedlichen Ergebnissen.  
>   
...
> [!IMPORTANT]Die **-G**-Option gilt nur für Azure SQL-Datenbank und Azure Data Warehouse. 
...
```
 ...
 Diese Option kann mehrmals verwendet werden: **-i***Eingabedatei* **-I***I Eingabedatei.*  
 ...
 **-I**  
 Legt die SET QUOTED_IDENTIFIER-Verbindungsoption auf ON fest. Die Standardeinstellung ist OFF. Weitere Informationen finden Sie unter [SET QUOTED_IDENTIFIER &#40;Transact-SQL&#41;](../Topic/SET%20QUOTED_IDENTIFIER%20\(Transact-SQL\).md).  
...
-   **!!** *Befehl*  
 ...
 Wenn die **-X**-Option angegeben wird, verhindert sie die Übergabe von Umgebungsvariablen an **sqlcmd**. Sie verhindert darüber hinaus die Ausführung des Startskripts, das mithilfe der SQLCMDINI-Skriptvariablen angegeben wurde. Weitere Informationen zu **sqlcmd**Skriptvariablen finden Sie unter [Verwenden von sqlcmd mit Skriptvariablen](../Topic/Use%20sqlcmd%20with%20Scripting%20Variables.md).  
...
## <a name="remarks"></a>Hinweise  
 ...
 Wenn Sie **sqlcmd** interaktiv verwenden möchten, geben Sie **sqlcmd** an der Eingabeaufforderung und eine oder mehrere der weiter oben in diesem Thema beschriebenen Optionen ein. Weitere Informationen finden Sie unter [Verwenden des Hilfsprogramms „sqlcmd“](../Topic/Use%20the%20sqlcmd%20Utility.md).  
...
-   Für **:Error**, **:Out** und **:Perftrace** sollten verschiedene **\<***Dateiname***>**-Parameter verwendet werden. Wird derselbe **\<***Dateiname***>**-Parameter verwendet, werden Eingaben von den Befehlen womöglich vermischt.  
...
###  <a name="a-nameoutputxmla-xml-output-format"></a><a name="OutputXML"></a> XML-Ausgabeformat  
...
###  <a name="a-nameoutputjsona-json-output-format"></a><a name="OutputJSON"></a> JSON-Ausgabeformat  
...
## <a name="see-also"></a>Siehe auch  
 [Starten des Hilfsprogramms „sqlcmd“](../Topic/Start%20the%20sqlcmd%20Utility.md)   
 [Ausführen von Transact-SQL-Skriptdateien mithilfe von „sqlcmd“](../Topic/Run%20Transact-SQL%20Script%20Files%20Using%20sqlcmd.md)   
 [Verwenden des Hilfsprogramms „sqlcmd“](../Topic/Use%20the%20sqlcmd%20Utility.md)   
 [Verwenden von „sqlcmd“ mit Skriptvariablen](../Topic/Use%20sqlcmd%20with%20Scripting%20Variables.md)   
 [Herstellen einer Verbindung mit dem Datenbankmodul mithilfe von „sqlcmd“](../Topic/Connect%20to%20the%20Database%20Engine%20With%20sqlcmd.md)   
 [Bearbeiten von SQLCMD-Skripts mit dem Abfrage-Editor](../Topic/Edit%20SQLCMD%20Scripts%20with%20Query%20Editor.md)   
 [Verwalten von Auftragsschritten](../Topic/Manage%20Job%20Steps.md)   
 [Erstellen eines CmdExec-Auftragsschritts](../Topic/Create%20a%20CmdExec%20Job%20Step.md)  
