...
---
# <a name="linking-access-applications-to-sql-server---azure-sql-db-accesstosql"></a>Verknüpfen der Access-Anwendungen mit SQLServer - Azure SQL-Datenbank (AccessToSQL)
...
Wenn Sie eine Access-Tabelle zu verknüpfen einer [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] oder SQL Azure-Tabelle, die Jet-Datenbankmodul speichert, Verbindungsinformationen und Tabellenmetadaten, aber die Daten werden gespeichert [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] oder SQL Azure. Auf diese Weise können die Access-Anwendungen, die für die Access-Tabellen ausgeführt werden, obwohl in der tatsächlichen Tabellen und Daten sind [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] oder SQL Azure.  
  
> [!NOTE]  
> Bei Verwendung von [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] Authentifizierung, Ihr Kennwort in Klartext auf der verknüpften Access-Tabellen gespeichert. Es wird empfohlen, mithilfe der Windows-Authentifizierung.  
...
Nachdem Sie die Tabellen verknüpft, werden die Tabellen in SSMA mit einem kleinen Linksymbol angezeigt. In Access werden die Tabellen mit einem Symbol "verknüpfte" angezeigt. Dies ist eine Kugel mit einem Pfeil darauf zeigen.  
  
Wenn Sie eine Tabelle in Access öffnen, werden die Daten abgerufen, einen Keyset-Cursor verwenden. Bei großen Tabellen bedeutet dies, dass alle, die Daten nicht gleichzeitig abgerufen werden. Wie Sie die Tabelle durchsuchen, wird Zugriff nach Bedarf weitere Daten abrufen.  
  
> [!IMPORTANT]  
> Um den Zugriff auf Tabellen mit Azure-Datenbank zu verknüpfen, benötigen wir SQL Server Native Client(SNAC) Version 10.5 oder höher.   
...
Wenn Sie die Access-Tabellen, um eine SQL Server-Instanz verknüpft haben, und klicken Sie dann auf der Sie die Links in eine andere Instanz ändern möchten, müssen Sie die Tabellen erneut verknüpfen.  
...
Wenn die [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] oder SQL Azure-Tabellendefinitionen geändert werden, können Sie aufheben und dann erneut verknüpfen die Tabellen in SSMA mithilfe der weiter oben in diesem Thema beschriebenen Verfahren. Oder aktualisieren Sie die Tabellen mithilfe von Access.  
...
Die folgenden Abschnitte, die in vorhandenen Access-Anwendungen auftreten können, nach dem Migrieren von Datenbanken aus Zugriff auf Probleme mit [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] oder SQL Azure und verknüpfen Sie die Tabellen, die Ursachen und Lösungen bereit.  
...
-   Die Anwendung hängt von Funktionen, die nicht in vorhanden [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] oder SQL Azure. Dies bewirkt, dass Jet, Tabellen, lokal zum Ausführen einer SELECT-Abfrage zu ziehen.  
...
**Lösung:** konvertieren Sie Pass-Through-Abfragen, gespeicherten Prozeduren oder Sichten die langsam ausgeführte Abfragen. Konvertieren in Pass-Through-Abfragen müssen die folgenden Probleme:  
...
### <a name="after-migration-some-queries-will-not-let-the-user-add-a-new-record"></a>Nach der Migration werden einige Abfragen nicht der Benutzer einen neuen Datensatz hinzufügen können  
